...
export function HyperLogLog(m: number) {
  function log2(x: number) {
    return Math.log(x) / Math.LN2;
  }

  function rightmostIndexOf1(hash: number, max: number) {
    var r = 1;
  ...
  var alpha_m = m == 16 ? 0.673 : m == 32 ? 0.697 : m == 64 ? 0.709 : 0.7213 / (1 + 1.079 / m);
  ...
  function add(hash: number) {
    ...
    const registerUpdated = w > currentRegister;
